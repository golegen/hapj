...
!function($, undefined){
	 ...
	 * @param {Object} opt 选项
	 * method: 触发的方法 可选值为hover、click、scroll，默认为click
	 * map:实现将触发元素关联到目标元素的映射函数
	 * trigger:触发后触发元素做出的响应函数
	 * target:触发后目标元素做出的响应函数
	 * cycleTime:自动循环的时间，默认为0
	 * tag:使用当前元素的指定标签的元素作为触发元素
	 * stopTop:停留在顶部时的上边距，仅在scroll模式下启用
	 * endTop:点击后定位到触发元素时偏移的距离，仅在scroll模式下启用
	 * disableClick:是否禁用click，默认为false，表示点击trigger的时候会自动滚动到对应位置
	 */
